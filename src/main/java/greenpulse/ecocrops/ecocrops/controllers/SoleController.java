...
public class SoleController {
    ...
    public ResponseEntity<?> createSole(@RequestBody Map<String, Object> requestBody) {
        try {
            ...
            if (!requestBody.containsKey("superficie") || !requestBody.containsKey("localisation") || !requestBody.containsKey("agriculteurId")) {
                return ResponseEntity.badRequest().body(Map.of(
                    "error", "Les champs 'superficie', 'localisation' et 'agriculteurId' sont obligatoires."
                ));
            }
            ...
            Integer agriculteurId = Integer.parseInt(requestBody.get("agriculteurId").toString());


            // Appeler le service pour créer la sole
            Sole sole = soleService.createSole(superficie, localisation, agriculteurId);
    ...
    @PreAuthorize("hasAuthority('agriculteur')")
    public ResponseEntity<?> getSolesByAgriculteur(@PathVariable Integer agriculteurId) {
        try {
            List<Sole> soles = soleService.getSolesByAgriculteurId(agriculteurId);
            if (soles.isEmpty()) {
                return ResponseEntity.status(204).body(Map.of(
                    "message", "Aucune sole trouvée pour l'agriculteur ID " + agriculteurId
                ));
            }
            return ResponseEntity.ok(soles);
        } catch (Exception e) {
